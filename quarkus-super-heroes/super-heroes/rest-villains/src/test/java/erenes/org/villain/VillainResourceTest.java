...
import erenes.org.hero.villain.Villain;
...
class VillainResourceTest {
    ...
    void shouldNotGetUnknownVillain(){
        ...
        given().pathParam("id", randomId).when().get("/api/villains/{id}")
            .then().statusCode(NO_CONTENT.getStatusCode());
    }

    @Test
    void shouldGetRandomVillain() {
        given().when().get("api/villains/random")
            .then()
    ...
    void shouldRemoveAnItem() {
        given().pathParam("id", villainId).when()
            .delete("/api/villains/{id}").then()
